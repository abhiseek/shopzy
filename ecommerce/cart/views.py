...
def order_form(request):
    if(request.method=="POST"):
        ...
        for i in c:
            total+=i.quantity*i.product.price
        total=int(total*100)
        client=razorpay.Client(auth=('rzp_test_1KXWb3Xzr4hBkR','QiQgkzrhLaHp7u9rvWZRcFoz')) #client connection using id and secret code of razorpay
...
def payment_status(request,u):
    ...
    if(request.method=="POST"):
        ...
        client=razorpay.Client(auth=('rzp_test_1KXWb3Xzr4hBkR','QiQgkzrhLaHp7u9rvWZRcFoz'))
        print(client)
        try:
